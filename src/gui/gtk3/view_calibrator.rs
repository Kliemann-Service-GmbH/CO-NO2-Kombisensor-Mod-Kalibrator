...
pub fn launch(sensor_type: SensorType, builder: &gtk::Builder, kombisensor: &Arc<Mutex<Kombisensor>>) -> Result<(), CalibError> {
    ...
    let id_button_messpunkt_nullgas: ::gui::gtk3::libc::c_ulong = button_messpunkt_nullgas.connect_clicked(clone!(builder, kombisensor, sensor_type => move |_| {
        // println!("Button Nullgas, {:?}", sensor_type)
        ::gui::gtk3::view_messpunkt::launch(GasType::Nullgas, &sensor_type, &builder, &kombisensor);
    }));

    let id_button_messpunkt_messgas: ::gui::gtk3::libc::c_ulong = button_messpunkt_messgas.connect_clicked(clone!(builder, kombisensor, sensor_type => move |_| {
        // println!("Button Messgas, {:?}", sensor_type)
        ::gui::gtk3::view_messpunkt::launch(GasType::Messgas, &sensor_type, &builder, &kombisensor);
    }));

    let id_button_calibrator_save: ::gui::gtk3::libc::c_ulong = button_calibrator_save.connect_clicked(clone!(builder, kombisensor, sensor_type => move |_| {
