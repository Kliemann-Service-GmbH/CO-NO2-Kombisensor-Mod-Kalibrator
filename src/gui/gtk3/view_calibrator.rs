...
fn callback_button_messpunkt_nullgas<T: AsRef<str>>(sensor_type: T, builder: &gtk::Builder, kombisensor: &Arc<Mutex<Kombisensor>>) {
    ::gui::gtk3::view_messpunkt::launch(sensor_type, &builder, &kombisensor);
}
...
pub fn launch<T: AsRef<str>>(sensor_type: T, builder: &gtk::Builder, kombisensor: &Arc<Mutex<Kombisensor>>) -> Result<(), CalibError> {
    ...
    let kombisensor_no2 = kombisensor.clone();
    ...
    match sensor_type.as_ref() {
        "NO2" => {
            ...
            button_messpunkt_nullgas.connect_clicked(clone!(builder, kombisensor_no2 => move |_| {
                callback_button_messpunkt_nullgas("NO2", &builder, &kombisensor_no2);
            }));
        },
        "CO" => {
